...
from schemas.canonical_paper import (
    PaperDocument,
...
def validate_paper_document(doc: PaperDocument) -> ExtractionQualityReport:
    ...
    if len(sections) < 3:
        ...
        # Check for sequence index numbers in section headings
        idx_seq = []
        for sec in sections:
            # Match Roman numerals (e.g. I., II., III., IV.) or standard numbers (e.g. 1., 2.)
        ...
        # Verify if sequence numbers are strictly ascending
        out_of_order = []
    ...
    # Compile sentences from all section contents
    for sec in sections:
        ...
        for sentence in sentences:
            # Normalize whitespace/casing for comparison
    ...
    for page_info in doc.pages:
        ...
        # Check if figures or tables are on this page (which justifies low text count)
        has_fig_or_tab = (
    ...
    if empty_pages:
        scorecard["empty_pages"] = ValidationMetric(
            status="WARNING",
            message=f"Suspiciously empty pages detected.",
