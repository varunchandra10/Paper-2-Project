...
from .pipeline_schemas import PaperMetadata
...
class Provenance(BaseModel):
    ...
    section: str = Field(description="The section name or header matching the block's parent")
    text_span: str = Field(description="Exact snippet text block matching the value")
...
class SourceValue(BaseModel):
    ...
    source: Provenance = Field(description="Provenance source coordinates")
    status: str = Field(
...
class Section(BaseModel):
    ...
    page_start: int = Field(description="The 1-based page number where this section starts")
    page_end: int = Field(description="The 1-based page number where this section ends")
...
class Figure(BaseModel):
    ...
    caption: str = Field(description="Full caption text associated with this figure")
    page: int = Field(description="1-based page number containing the figure")
...
class Table(BaseModel):
    ...
    caption: str = Field(description="Full caption text associated with this table")
    content_markdown: str = Field(description="Markdown-formatted tabular string representation")
...
class Equation(BaseModel):
    ...
    page: int = Field(description="1-based page number containing the equation")
...
class Algorithm(BaseModel):
    ...
    caption: str = Field(description="Caption text for this algorithm")
    pseudocode: str = Field(description="The raw pseudocode lines or logic text block")
    page: int = Field(description="1-based page number containing the algorithm")
...
class Citation(BaseModel):
    ...
    target_text: str = Field(description="Context text immediately surrounding the citation marker")
    source: Provenance = Field(description="Provenance coordinate location of the citation marker")
...
class PageInfo(BaseModel):
    ...
    width: float = Field(description="Page width dimension in points")
    height: float = Field(description="Page height dimension in points")
    character_count: int = Field(description="Total characters extracted from this page")
...
class PaperDocument(BaseModel):
    ...
    metadata: PaperMetadata = Field(description="Clean paper metadata (title, authors list, abstract)")
...
class ExtractionQualityReport(BaseModel):
    ...
    valid: bool = Field(description="True if the document contains zero 'ERROR' flags")
